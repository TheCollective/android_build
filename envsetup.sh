...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the AOCP github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var AOCP_VERSION)
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.aocp.device=$AOCP_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $AOCP_BUILD, run away!"
    fi
}
...
function installboot()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cm.device=$AOCP_BUILD");
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $AOCP_FIXUP_COMMON_OUT ]; then
