...
def get_from_manifest(devicename):
    ...
    for localpath in lm.findall("project"):
        if re.search("aocp_device_.*_%s$" % device, localpath.get("name")):
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "revision": "jellybean2", "remote": "github", "name": "TheCollective/%s" % repo_name })
    ...
    f = open('.repo/local_manifests/aocp_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()

def fetch_dependencies(repo_path):
    print 'Looking for dependencies'
    dependencies_path = repo_path + '/aocp.dependencies'
    syncable_repos = []
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("aocp_device_") and repo_name.endswith("_" + device):
            print "Found repository: %s" % repository['name']
            manufacturer = repo_name.replace("aocp_device_", "").replace("_" + device, "")
